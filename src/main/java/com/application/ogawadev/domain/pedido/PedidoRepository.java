package com.application.ogawadev.domain.pedido;
...
//	@Query("SELECT p FROM Pedido p WHERE p.restaurante.id = ?1 ORDER BY p.data DESC")
//	public List<Pedido> listPedidosByRestaurante(Integer restauranteId);
...
//	@Query("SELECT i.itemCardapio.nome, COUNT(i.itemCardapio.id), SUM(i.preco) FROM Pedido p INNER JOIN p.itens i WHERE p.restaurante.id = ?1 AND p.data BETWEEN ?2 AND ?3 GROUP BY i.itemCardapio.nome")
