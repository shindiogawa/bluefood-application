package com.application.ogawadev.application.util;
...
import com.application.ogawadev.infrastructure.web.security.LoggedUser;
...
public class SecurityUtils {
	...
	public static Cliente loggedCliente() {
		...
		if(loggedUser == null) {
			...
			throw new IllegalStateException("Nao existe um usuario logado");
			
		}
		
		if(!(loggedUser.getUsuario() instanceof Cliente)) {
			throw new IllegalStateException("O usuario logado nao e um cliente");
		}
...
public static Restaurante loggedRestaurante() {
		...
		if(!(loggedUser.getUsuario() instanceof Restaurante)) {
			throw new IllegalStateException("O usuario logado nao e um restaurante");
		}
